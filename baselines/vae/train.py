...
parser.add_argument('--input_file', type=str, action='store', default='orders',
...
orig_df = load_table(args.input_file)
...
if os.path.exists(args.data_output_dir+'data.pkl'):
    ...
    x_train, x_test = transform_forward(
        args, df, num_cols, cat_cols, encoding_type)
    pickle.dump((x_train, x_test), open(
        args.data_output_dir+'data.pkl', 'wb'), protocol=4)
