...
parser.add_argument('--input_file', type=str, action='store',
                    default=f'/home/clb/AQP/data/{DATASET_NAME}.csv')
parser.add_argument('--output_dir', type=str, action='store',
                    default='outputs/{}/'.format('vae-orders'))
...
if os.path.exists(args.data_output_dir + 'data.pkl'):
    x_train, x_test = pickle.load(
        open(args.data_output_dir + 'data.pkl', 'rb'))
else:
    print("Training Data Doesn't exist.")
    exit(0)

print(x_train.shape)
x_train_sampled = x_train[np.random.randint(
    x_train.shape[0], size=args.num_samples), :]
...
with torch.no_grad():
    ...
    while tgt_count > 0:
        ...
        # Probabilitiy of z being sampled from normal prior.
        p_z = normal.log_prob(rep_z)

        # P(x|z)
        x_cap = model.decode(rep_z)  # Output based on the obtained z.
        x_distr = torch.distributions.normal.Normal(torch.mean(
            x_cap, axis=0), 1)  # Distribution with x_cap as mean
        ...
        a = torch.exp(T + torch.mean(p_z, axis=-1) +
                      torch.mean(p_x_z, axis=-1) - torch.mean(q_z_x, axis=-1))
