            ...
            if lb+1 == ub:
                ...
                if k in cat_cols:
                    ...
                    try:
                        ...
                        print(index, lb, ub, np.argmax(
                            x_decoded[lb:ub]), uniques)
...
class VAE(nn.Module):
    ...
    def rejection_batchwise(self, x, mu, logvar, epoch):
        ...
        while tgt_count >= 0:
            ...
            accept_prob = torch.zeros(
                (x.shape[0],)) + 0.9  # Accept Prob of 0.9
            std = torch.exp(0.5*logvar)
            eps = torch.randn_like(std)
...
#         print(q_z_x.shape)
        ...
        T = np.log(0.9) - torch.mean(p_z, axis=-1) - \
...
def loss_function(recon_x, x, mu, logvar, org_input_dim):
    BCE = F.binary_cross_entropy(
        recon_x, x.view(-1, x.shape[1]), reduction='sum')
    ...
    with torch.no_grad():
        for step in range(nsteps):
            ...
            test_loss += loss_function(recon_batch,
                                       data, mu, logvar, org_input_dim).item()
    test_loss /= N
