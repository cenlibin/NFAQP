...
class VEGASMap:
    ...
    def __init__(self, N_intervals, dim, alpha=0.5, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')) -> None:
        ...
        self.dx_edges = torch.ones(size=(self.dim, self.N_intervals), device=device) / self.N_intervals
        x_edges_per_dim = torch.linspace(0.0, 1.0, N_edges, dtype=self.dtype).to(self.device)
        self.x_edges = torch.repeat_interleave(x_edges_per_dim.view(1, N_edges), repeats=self.dim, dim=0)
    ...
    def get_X(self, y):
        ...
        full_range_y = y * float(self.N_intervals)
        ID = torch.floor(full_range_y).type(torch.int64)
        offset = full_range_y - ID

        
        x_mapping_from_y = torch.empty_like(y)
        for i in range(self.dim):
            ID_i = ID[:, i]
            x_mapping_from_y[:, i] = self.x_edges[i, ID_i] + self.dx_edges[i, ID_i] * offset[:, i]
        return x_mapping_from_y
    ...
    def accumulate_weight(self, y, jf_vec2):
        ...
        ID = self._get_interval_ID(y)
        for i in range(self.dim):
            _add_at_indices(self.weights[i], ID[:, i], jf_vec2)
            _add_at_indices(self.counts[i], ID[:, i], ones)
