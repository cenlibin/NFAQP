...
class VegasAQP:
    ...
    def __init__(
        ...
        self._N_starting = n_sample_points // (max_iteration + 5)
        ...
        self.stratification_sampler = VEGASStratification(
            self._N_increment,
            dim=self.dim,
            beta=self.beta
        )
    ...
    def integrate(
        ...
        while it < self.max_iteration:
            ...
            # each iteration
            neval = self.stratification_sampler.get_NH(self._N_starting)
            # Stratified sampling points y and transformed sample points x
            y = self.stratification_sampler.get_Y(neval)
            ...
            jf, jf2, target_col_vals = self.stratification_sampler.accumulate_weight(neval, jf_vec, target_col_vals)

            ih = jf * self.stratification_sampler.V_cubes / neval
            sel = ih.sum()
            ...
            # Collect results
            sig2 = (jf2 * (self.stratification_sampler.V_cubes ** 2) / neval - ih ** 2).detach().abs()
            sig2 = (sig2 / neval).sum()
            ...
            if self.use_grid_improve:
                self.map.update_map()
            self.stratification_sampler.update_DH()
    ...
    def groupby_integrate(
        ...
        groupby_volume = groupby_size.prod(dim=1)
        ...
        self.batch_stratification_sampler = VEGASMultiStratification(
            n=self.batch_size, 
        ...
        while it < self.max_iteration:
            ...
            # each iteration
            # (batch, N_cubes)
            batch_n_points_per_cube = self.batch_stratification_sampler.get_NH(self._N_starting)  # 分桶， 每个桶里面n_i个采样点
            # Stratified sampling points y and transformed sample points x
            y = self.batch_stratification_sampler.get_Y(batch_n_points_per_cube)
            y = y.permute(2, 0, 1)
            ...
            batch_f_eval = self.fn(groupby_x).view(self.batch_size, -1) # (batch, n_evals)
            ...
            jf, jf2, target_col_vals = self.batch_stratification_sampler.accumulate_weight(batch_n_points_per_cube, jf_vec, target_col_vals)

            v_cubes = self.batch_stratification_sampler.V_cubes

            # (batch, n_chunks) * [1] / (batch, n_chunks) * [1]
            ih = jf * v_cubes / (batch_n_points_per_cube * 1.0)
            sel = ih.sum(1)
            target_col_prob = ih / sel.view(-1, 1)      # normalize the prob density to prob
            prob_vals = target_col_prob * target_col_vals
            ave = prob_vals.sum(1)
            var = ((target_col_prob * (target_col_vals - ave.view(-1, 1))) ** 2).sum(1)
            
            sig2 = (jf2 * (v_cubes ** 2) / batch_n_points_per_cube - ih ** 2).detach().abs()
            sig2 = (sig2 / batch_n_points_per_cube).sum(1)
            ...
            self.batch_stratification_sampler.update_DH()
        ...
        den = (1.0 / sigs).sum(1)
        sel = (sels.permute(1, 0) / sigs).sum() / den
        ave = (aves.permute(1, 0) / sigs).sum() / den
        var = (vars.permute(1, 0) / sigs).sum() / den
    ...
    def batch_get_dx_x_edges_vec(self, target_map, n_edges, y_lims):
        ...
        ret_dx_edges = ret_x_edges[:,1:,:] -  ret_x_edges[:, :-1, :]
        # ret_dx_edges = ret_x_edges[1:, :] - ret_x_edges[:-1, :]
        assert ret_dx_edges.shape == (self.batch_size, n_edges-1, self.dim)
