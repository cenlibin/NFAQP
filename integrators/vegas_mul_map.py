...
class VEGASMultiMap:
    ...
    def __init__(self, n, N_intervals, dim, alpha=0.5, device=torch.device('cpu' if not torch.cuda.is_available() else 'cuda')) -> None:
        self.n = n
        self.dim = dim
    ...
    def get_X(self, y):
        ...
        integrator_id = torch.repeat_interleave(integrator_id.reshape(1, -1, 1), ID.shape[2], dim=2)
        integrator_id = torch.repeat_interleave(integrator_id, ID.shape[0], dim=0)
    ...
    def accumulate_weight(self, y, jf_vec2):
        ...
        # (dim, n, bla)
        jf_vec2 = torch.repeat_interleave(jf_vec2, self.dim, dim=0)
        jf_vec2 = jf_vec2.view(-1)

        ones = ones.view(-1)
        ones = torch.repeat_interleave(ones, self.dim, dim=0)
    ...
    def update_map(self):
        ...
        dim_id = torch.arange(self.dim)
        dim_id = torch.repeat_interleave(dim_id.reshape(-1, 1, 1), delta_d_multiples.shape[2], dim=2)
        dim_id = torch.repeat_interleave(dim_id, delta_d_multiples.shape[1], dim=1)

        integrator_id = torch.arange(delta_d_multiples.shape[1])
        integrator_id = torch.repeat_interleave(integrator_id.reshape(1, -1, 1), delta_d_multiples.shape[2], dim=2)
        integrator_id = torch.repeat_interleave(integrator_id, delta_d_multiples.shape[0], dim=0)
