...
class VEGASMultiStratification:
    ...
    def __init__(self, n, N_increment, dim, beta=0.75, device=torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        ...
        # (self.n, self.N_cubes)
        self.JF = torch.zeros([self.n, self.N_cubes], dtype=self.dtype, device=self.device)
        self.JF2 = torch.zeros([self.n, self.N_cubes], dtype=self.dtype, device=self.device)

        # dampened counts
        # (self.n, self.N_cubes)
        self.dh = torch.ones([self.n, self.N_cubes], device=self.device, dtype=self.dtype) * 1.0 / self.N_cubes



        # current index counts as floating point numbers
        # (self.n, self.N_cubes)
        self.strat_counts = torch.zeros([self.n, self.N_cubes], dtype=self.dtype, device=self.device)
    ...
    def accumulate_weight(self, nevals, weight_all_cubes, target_col_vals):
        ...
        N_cubes_arange_n = torch.repeat_interleave(N_cubes_arange.view(1, -1), self.n, dim=0).view(-1)
        indices = torch.repeat_interleave(N_cubes_arange_n, nevals.view(-1), dim=0)
        ...
        chunk_target_vals = anp.zeros([self.n, self.N_cubes], dtype=self.dtype, like=self.backend)

        integrator_id = torch.arange(self.n, dtype=int)
        integrator_id = integrator_id.reshape(-1, 1)

        # (n, total_bla)
        integrator_id = torch.repeat_interleave(integrator_id, indices.shape[1], dim=1)

        JF_shape = self.JF.shape
        weight_all_cubes = weight_all_cubes.view(-1)
        target_col_vals = target_col_vals.view(-1)
        # (0,0,0……, n-1, n-1)
    ...
    def _get_indices(self, idx):
        ...
        if self.backend == "torch":
            ...
            # instead
            points = torch.floor(torch.div(points, strides))
    ...
    def get_Y(self, nevals):
        ...
        assert self.N_cubes == nevals.shape[1]
        nevals_arange = torch.arange(self.N_cubes, dtype=int)


        N_cubes_arange_n = torch.repeat_interleave(nevals_arange.view(1, -1), self.n, dim=0).view(-1)


        indices = torch.repeat_interleave(N_cubes_arange_n, nevals.view(-1))
        ...
        # )
        random_uni = torch.rand(size=[positions.shape[0], positions.shape[1], positions.shape[2]], dtype=self.dtype, device=self.device)
