...
class VEGAS:
    ...
    def integrate(
        ...
        # Note that a larger number of intervals may lead to problems if only few eval are allowed
        # Paper section II B
        N_intervals = int(max(2, self._N_increment // 10))  # for small N intervals set 2
        self.map = VEGASMap(N_intervals, self._dim)
        ...
        while True:
            ...
            print(f"vegas iteration:{self.it} uncertainty:{acc.item():.5f} result:{self.results[-1]}")
    ...
    def _check_abort_conditions(self):
        ...
        # Adjust number of eval if Chi square indicates instability
        # EQ 32
        if chi2 / 5.0 < 1.0:
    ...
    def _warmup_grid(self, warmup_N_it=5, N_samples=1000):
        ...
        for warmup_iter in range(warmup_N_it):
            ...
            yrnd = torch.rand([N_samples, self._dim]) * 0.999999
            x = self.map.get_X(yrnd)
            f_eval = self._fn(x).squeeze()
    ...
    def _run_iteration(self):
        ...
        jf, jf2, _ = self.strat.accumulate_weight(neval, jf_vec, jf_vec)  # update strat
